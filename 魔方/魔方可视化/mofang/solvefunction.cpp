...
string toString() // 将初始状态转化为字符串作为初始状态̬
{
    string str;
    for (int j = 0; j < 3; j++)
        for (int k = 0; k < 3; k++)
            str += get(color_back[j * 3 + k]) + '0';
    for (int i = 0; i < 3; i++)
    {
        for (int j = 0; j < 3; j++)
           str += get(color_left[i * 3 + j]) + '0';
        for (int j = 0; j < 3; j++)
            str += get(color_up[i * 3 + j]) + '0';
        for (int j = 0; j < 3; j++)
            str += get(color_right[i * 3 + j]) + '0';
    }

    for (int j = 0; j < 3; j++)
        for (int k = 0; k < 3; k++)
            str += get(color_front[j * 3 + k]) + '0';

    for (int j = 0; j < 3; j++)
        for (int k = 0; k < 3; k++)
            str += get(color_down[j * 3 + k]) + '0';
...
void solveFunction()
{
    ...
    // 动作与动作之间可以存在空格，但数字后面必须紧跟‘+’或‘-’
        ...
        for (int i = top; i; i--)
        {
            ...
            string res = to_string(stk[i] / 2);
            res += (t ? '-' : '+');
            printf("%s\n", res.c_str());
            input_action::do_action(res);
        }
        cout << endl;
}
