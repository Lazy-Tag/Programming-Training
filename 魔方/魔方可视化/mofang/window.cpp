...
Window::Window()
{
    ...
    for(int i=0;i<0;i++)
        quene_transform.push_back(qrand()%12);
...
{
    mouseLastPosition=QVector2D(event->localPos());
    mouseOnPress=true;
...
void Window::mouseMoveEvent(QMouseEvent *event)
{
    if(mouseOnPress)
    {
        QVector2D diff = QVector2D(event->localPos()) - mouseLastPosition;
...
void Window::solve()
{
    ...
    for(int i=0;i<solve_action.size();i++){
        ...
        case 'U':
            if(i+1 < solve_action.size() && solve_action[i+1]=='\'' ){
                push_action+="5";
                push_action+="-";
                break;
            }
            if(i+1 < solve_action.size() && solve_action[i+1]=='2' ){
                push_action+="5";
                push_action+="+";
                push_action+="5";
                push_action+="+";
