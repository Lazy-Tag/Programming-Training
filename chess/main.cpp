...
#define DEBUG 0 // 调试宏
#define MAXTIME 910 // 最大运行时间
#define MORECHESS 0 // 跳过大于五个棋子的情况
#define CHENG5 1000000 // 各种棋型分数定义
#define HUO4 10000
#define CHONG4 1000
#define HUO3 800
#define TIAOHUO3 700
...
void print() // 调试函数
{
    for(int i = 0; i < N; i ++ )
    {
        ...
        {
            int t = 2;
            if(graph[i][j] >= 0) printf("%3d", graph[i][j]);
            else printf("%3d", t);
        }
...
int evaluate(int x, int y, int p) // 每颗棋子对应分数
{
    ...
    for(int i = 0; i < 8; i += 2) // 四个方向建成字符串
    {
        ...
        for(int k = 1; k <= 4; k ++ )
        {
            ...
            else if(graph[a][b] == (p ^ 1)) str += '-';
            else if(graph[a][b] == p) str += '1';
            else str += '0';
        }
        ...
        if(str.find("111111") != string :: npos) res += MORECHESS; // 判断棋型的类型
        else if(str.find("11111") != string :: npos) res += CHENG5;
        else if(str.find("011110") != string :: npos) res += HUO4;
        else if(str.find("01111-") != string :: npos ||
                str.find("-11110") != string :: npos ||
                str.find("10111") != string :: npos ||
                str.find("11011") != string :: npos ||
                str.find("11101") != string :: npos ) res += CHONG4;
        else if(str.find("01110") != string :: npos ) res += HUO3;
        else if(str.find("1011") != string :: npos ||
                str.find("1101") != string :: npos ) res += TIAOHUO3;
                ...
                str.find("-01110-") != string :: npos) res += MIAN3;
        else if(str.find("001100") != string :: npos ||
                str.find("01010") != string :: npos ||
                str.find("1001") != string :: npos ) res += HUO2;
...
int h() // 评估棋局分数
{
    ...
    for(int i = 0; i < N; i ++ )
        for(int j = 0; j < N; j ++ )
            if(graph[i][j] == player) res += evaluate(i, j, player);
            else if(graph[i][j] == nextplayer) res -= evaluate(i, j, nextplayer);
    return res;
}
...
bool miniMax(int u, int depth, int BEGIN) // minimax求下一步棋的位置
{
    ...
    if(u == player) reverse(node, node + cnt);
    LL self = MAP;
    LL parent = toParent[MAP];
    double v = chessBoard[parent].y;
    if(!chessBoard.count(self)) // 初始化
    {
        if(u == player) chessBoard[self] = {0, -INF};
        else chessBoard[self] = {0, INF};
    }
    for(int i = 0; i < cnt; i ++ ) // 遍历每种情况
    {
        ...
        graph[a][b] = u;
        MAP ^= zobristBlock[a][b];
        if(u == player) MAP ^= zobristPlayer[a][b];
        else MAP ^= zobristNextPlayer[a][b];
        int option = a * N + b;
        S.insert(option);
        if (u == player)
        {
            if(chessBoard.count(MAP) && chessBoard[MAP].x < depth) goto R1; // 置换表优化
            if(miniMax(u ^ 1, depth - 1, BEGIN)) return true;
            R1:
            graph[a][b] = -1;
            ...
            S.erase(option);
            if (chessBoard[self].y > v) return false; // AlphaBeta剪枝
        }
        else
        {
            if(chessBoard.count(MAP) && chessBoard[MAP].x < depth) goto R2; // 置换表优化
            if(miniMax(u ^ 1, depth - 1, BEGIN)) return true;
            R2:
            graph[a][b] = -1;
            if (chessBoard[MAP] < chessBoard[self])
...
int main()
{
    ...
    if(!flag) cout << 7 << ' ' << 7 << endl; // 如果棋盘上没棋子，直接下中元
    else
    {
        for(int i = 0; i < N; i ++ )
            for(int j = 0; j < N; j ++ )
            {
                int t = graph[i][j];
                if(t == -1) MAP ^= zobristBlock[i][j];
                else if(t == player) MAP ^= zobristPlayer[i][j];
                else MAP ^= zobristNextPlayer[i][j];
            }
        backup = MAP; // 初始棋盘哈希值
        toParent[MAP] = 0;
        if(DEBUG) print();
        int depth = 2;
        clock_t BEGIN = clock();
        while(!miniMax(player, depth, BEGIN)) depth += 2 ; // 迭代加深搜索
